...
namespace {
  ...
  point predict_step(const point &start, direction dir) {
    switch (dir) {
    case north:
      return point { start.i - 1, start.j };
    case south:
      return point { start.i + 1, start.j };
    case east:
      return point { start.i, start.j + 1 };
    case west:
      return point { start.i, start.j - 1 };
    }
  }
...
optional<point> snake::walk(const point &p) {
  body_.push(p);
...
point playing_field::to_coord(size_t idx) const {
  return point { idx / width_, idx % width_ };
}
...
tile playing_field::get(const point &p) const {
  return data_[from_coord(p)];
}
...
bool playing_field::is_full() const {
  return free_spots_.empty();
}

optional<point> playing_field::find_empty(range_rng rng) const {
  if (is_full()) {
    return optional<point>();
  }
  std::set<size_t>::const_iterator it(free_spots_.begin());
  std::advance(it, rng(0, free_spots_.size()));
  return to_coord(*it);
}

unique_ptr<game_state> game_state::create(size_t height, size_t width,
                                          point snake_origin,
  ...
  auto s = std::make_unique<snake>(snake_origin);
  return std::unique_ptr<game_state>(new game_state(std::move(field), std::move(s),
                                                    growth_rate, rng));
}
...
tile_updates game_state::step(direction d) {
  ...
  case tile::fruit:
    ...
    // fallthrough
  case tile::empty:
    auto maybe_tail = snake_->walk(new_head);
    if (maybe_tail) {
  ...
  if (grow) {
    auto maybe_fruit = field_->find_empty(rng_);
    if (maybe_fruit) {
